...
class StooqGoodsProvider(BaseProvider):
    _URL = "https://stooq.com/q/l/"

    @property
    def name(self: StooqGoodsProvider) -> str:
        return "stooq"

    async def _do_fetch(
        self: StooqGoodsProvider,
    ...
    async def _throttle_request(self: StooqGoodsProvider) -> None:
        delay_seed = secrets.randbelow(1_000_000) / 1_000_000
        delay = 0.5 + delay_seed
        await asyncio.sleep(delay)

    def _parse_csv(self: StooqGoodsProvider, text: str) -> Mapping[str, dict[str, str]]:
